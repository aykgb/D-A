...
void traverse_tree_postorder_nonrecursive(BinTreeNode *root, Visit visit) {
  ...
  while (!st.empty()) {
    ...
    if ((iter->lchild == nullptr && iter->rchild == nullptr) ||
        (iter_pre != nullptr &&
         (iter->lchild == iter_pre || iter->rchild == iter_pre))) {
      visit(iter);
      st.pop();
      iter_pre = iter;
    } else {
...
int test_bin_tree() {
  ...
  traverse_tree_preorder(
      root, [](BinTreeNode *node) { std::cout << node->val << " "; });
  std::cout << std::endl;
  traverse_tree_preorder_nonrecursive(
      root, [](BinTreeNode *node) { std::cout << node->val << " "; });
  std::cout << std::endl;
  traverse_tree_inorder(
      root, [](BinTreeNode *node) { std::cout << node->val << " "; });
  std::cout << std::endl;
  traverse_tree_inorder_nonrecursive(
      root, [](BinTreeNode *node) { std::cout << node->val << " "; });
  std::cout << std::endl;
  traverse_tree_postorder(
      root, [](BinTreeNode *node) { std::cout << node->val << " "; });
  std::cout << std::endl;
  traverse_tree_postorder_nonrecursive(
      root, [](BinTreeNode *node) { std::cout << node->val << " "; });
  std::cout << std::endl;
  destroy_tree(root);
...
TreeNode *create_tree_v2(vector<string> &arr, size_t pos) {
  TreeNode *root = nullptr;
  if (pos < arr.size()) {
    auto val = arr[pos];
    if (val != "null") {
      root = new TreeNode(std::atoi(val.c_str()));
      root->left = create_tree_v2(arr, 2 * pos + 1);
      root->right = create_tree_v2(arr, 2 * pos + 2);
    }
  }
