...
class LFUCache {
  ...
  std::unordered_map<Key, CacheNode *> map_key2node;
 ...
 public:
  void dump() {
    ...
    std::cout << " dump cache:\n";
    for (auto &e : map_key2node) {
      std::cout << " @key:" << e.first << " value:" << e.second->value
 ...
 public:
  ...
  int get(int key) {
    if (map_key2node.count(key) == 0) {
      return -1;
    }

    CacheNode *node = map_key2node[key];
    // 从map_freq_list中移除key对应的信息
    map_freq2key_list[node->frequency].erase(node->iter);

    node->frequency++;

    // 更新到map_freq_list和map_list_iter中
    map_freq2key_list[node->frequency].push_front(key);
    node->iter = map_freq2key_list[node->frequency].begin();
  ...
  void put(int key, int value) {
    ...
    if (size == capacity) {  // cache 已满，淘汰一个node
      ...
      auto node = map_key2node[key];
      delete node;

      map_key2node.erase(key);
      size--;
    }

    // 加入一个新的node
    min_freq = 1;
    map_key2node[key] = new CacheNode(key, value);
    map_freq2key_list[min_freq].push_front(key);
    map_key2node[key]->iter = map_freq2key_list[min_freq].begin();
    size++;
  }
};
